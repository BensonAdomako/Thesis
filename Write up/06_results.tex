...
\begin{threeparttable}
\input{tables/regression_results_robustness}
\end{threeparttable}
\end{table}
...
\subsection{Robustness Check 2: Instrumental Variables}

A fundamental concern with using ML-predicted yields as the primary regressor is that while predictions are constructed from exogenous satellite and weather inputs, the satellite vegetation signal could in principle reflect conflict-induced agricultural abandonment at a sufficient spatial scale. To directly test whether observed yields---the economically relevant quantity---are endogenous to conflict, and whether the predicted-yield regressor recovers the correct causal estimate, I implement a 2SLS specification in which log observed yield (from GROW-Africa administrative records) is the endogenous variable and log ML-predicted yield is the instrument.

Table~\ref{tab:iv} reports the first-stage and second-stage results. The IV sample is restricted to the four countries for which GROW-Africa provides GID_2-level observed yield records: Burkina Faso, Ethiopia, Malawi, and Niger (N~=~778, 131 districts). Mali, Nigeria, and Tanzania have only national-level GROW-Africa records and are therefore excluded.

\begin{table}[H]
\centering
\caption{Robustness Check: IV/2SLS with Observed Yield and Predicted Yield Instrument}
\label{tab:iv}
\begin{threeparttable}
\input{tables/regression_results_iv}
\end{threeparttable}
\end{table}

The first stage is strong: the Wald F-statistic on the excluded instrument is 30.8, well above the Stock-Yogo threshold of 10, confirming that log predicted yield is a relevant instrument for log observed yield conditional on district and year fixed effects. The Wu-Hausman endogeneity test rejects exogeneity of observed yield at $p = 0.004$, confirming that OLS on observed yields is biased --- as expected given that conflict destroys agricultural output and attenuates OLS estimates toward zero.

The TWFE OLS estimate on observed yield in the IV sample is $\hat{\beta} = -0.210$ ($p = 0.025$). The 2SLS estimate corrects upward in magnitude to $\hat{\beta} = -0.687$ ($\text{SE} = 0.236$, $p = 0.004$). Two features of this result are notable. First, the direction of bias is consistent with the reverse-causality hypothesis: conflict depresses observed yields, attenuating OLS estimates toward zero, and the IV correction amplifies the negative estimate. Second, the 2SLS estimate of $-0.687$ is remarkably close to the main preferred estimate of $-0.666$ from the predicted-yield specification on the full 18,096-observation panel. This convergence across two entirely different identification strategies --- direct use of predicted yields as a regressor versus instrumenting observed yields with predicted yields --- provides strong cross-validation that the main result is not an artefact of the measurement approach.

\subsection{Robustness Check 3: Spatial Lag Control}
...
\begin{threeparttable}
\input{tables/regression_results_spatial_lag}
\end{threeparttable}
\end{table}
...
\begin{threeparttable}
\input{tables/regression_results_conflict_types}
\end{threeparttable}
\end{table}
...
\label{tab:aez}
\begin{threeparttable}
\input{tables/regression_results_aez}
\end{threeparttable}
\end{table}
...
Figure~\ref{fig:aez_binscatter} provides visual confirmation of this reversal. The figure plots binned means of demeaned log(1\,+\,conflict) against demeaned log predicted yield---both residualised of district and year fixed effects---separately for Sahel and Non-Sahel zones. The Non-Sahel panel shows a clearly downward-sloping relationship, while the Sahel panel shows a clearly upward-sloping one. The contrast in slopes is directly interpretable as the interaction coefficient reported in Column~(3) of Table~\ref{tab:aez}.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig7_aez_binscatter.pdf}
\caption{Yield--Conflict Relationship by Agro-Ecological Zone (Binned Scatter, Within-District Variation). Both variables demeaned by Admin-2 and year fixed effects. Lines are OLS fits with 95\% confidence bands. Point size proportional to bin observation count.}
\label{fig:aez_binscatter}
\end{figure}
...
Figure~\ref{fig:country_coefs} presents per-country TWFE OLS log-log estimates as a forest plot. Point estimates are negative for Burkina Faso, Mali, and Malawi, and near zero for Nigeria and Tanzania, though none of the individual country estimates achieves conventional significance. Ethiopia is the only country with a positive and statistically significant estimate, driven by the Tigray and Amhara conflict episodes discussed below. The Sahel countries (orange) tend toward zero or positive point estimates, while Non-Sahel countries (blue) cluster near or below zero, visually anticipating the AEZ interaction result.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig6_country_coefs.pdf}
\caption{Per-Country TWFE OLS Log-Log Estimates: Effect of Predicted Yield on Log Conflict Events. Thick bars: 90\% CI; thin bars: 95\% CI. Orange = Sahel; Blue = Non-Sahel.}
\label{fig:country_coefs}
\end{figure}
...
\begin{threeparttable}
\input{tables/regression_results_bycountry_ols}
\end{threeparttable}
\end{table}
...
\begin{threeparttable}
\input{tables/regression_results_bycountry_poisson}
\end{threeparttable}
\end{table}
